...
export default function HandoffBody() {
  return (
    <>
      ...
      <section className="space-y-3">
        ...
        <p className="text-muted-foreground">
          ...
          <code className="rounded bg-muted px-1">NEXT_PUBLIC_API_V1_URL</code> с реальным URL. OAuth Яндекса: в{" "}
          <code className="rounded bg-muted px-1">.env</code> задаются{" "}
          <code className="rounded bg-muted px-1">YANDEX_OAUTH_CLIENT_ID</code>,{" "}
          <code className="rounded bg-muted px-1">YANDEX_OAUTH_CLIENT_SECRET</code>,{" "}
          <code className="rounded bg-muted px-1">YANDEX_OAUTH_REDIRECT_URI</code> (как в консоли OAuth), опционально{" "}
          <code className="rounded bg-muted px-1">YANDEX_OAUTH_UI_DEFAULT_REDIRECT</code> (куда вернуть браузер после
          успешного OAuth, если не передан <code className="rounded bg-muted px-1">ui_redirect</code>) и{" "}
          <code className="rounded bg-muted px-1">YANDEX_OAUTH_TOKEN</code> только для скриптов. Для отладки:{" "}
          <code className="rounded bg-muted px-1">GET /api/v1/health</code> — поля{" "}
          <code className="rounded bg-muted px-1">yandex_oauth_redirect_uri</code> (callback) и{" "}
          <code className="rounded bg-muted px-1">yandex_oauth_ui_default_redirect</code> (куда вернёт UI после OAuth,
          если не задан <code className="rounded bg-muted px-1">ui_redirect</code>).
        </p>
      </section>
...
# OAuth: в ответе /health поля yandex_oauth_redirect_uri (callback) и yandex_oauth_ui_default_redirect (куда вернёт UI)`}
            </pre>
          </div>
          <div>
