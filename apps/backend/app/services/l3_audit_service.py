...
class L3AuditService:
    def __init__(self, session: AsyncSession) -> None:
        ...
        self._registry = build_l3_rule_registry()

    async def run_manual_l3_audit(
        self,
        *,
        account_id: UUID,
        ...
        l3_rules = [r for r in rules if r.enabled and r.level == "L3" and r.check_type == "deterministic"]
        ...
        try:
            ...
            for rule in l3_rules:
                ...
                raw_rule["rule_code"] = rule.rule_code
                raw_rule.setdefault("recommendation_ru", rule.rule_name)
                raw_rule.setdefault("required_utm_params", settings.required_utm_params_list)
